...
conditions["$in"] = function(conds, k, v, opt, args)
	...
	if type(args[1]) == "string" then
		...
		for i,arg in ipairs(args) do
			tmps[i] = string.format([["%s"]], string.dbencode(arg)) 
		end
		args = tmps
	...
	table.insert(conds, string.format("`%s` IN (%s)",k, table.concat(args, ", ")))
end

conditions["$like"] = function(conds, k, v, opt, args)
	if type(args) == "string" then
		table.insert(conds, string.format([[`%s` LIKE "%s"]], k, string.dbencode(args)))
	elseif type(args) == "number" then
		table.insert(conds, string.format([[`%s` LIKE %s]], k, args))
	else
		assert(false)
	end
end
...
conditions["$nin"] = function(conds, k, v, opt, args)
	...
	table.insert(conds, string.format([[`%s` NOT IN (%s)]],k, table.concat(args, ", ")))
end

conditions["$gte"] = function(conds, k, v, opt, args)
	assert(type(args) == "number")
	table.insert(conds, string.format([[`%s` >= %s]], k, args))
end
	...
	for i,arg in ipairs(v) do
		...
		for k,v in pairs(arg) do
			if type(v) == "string" then
				table.insert(tmps, string.format([[`%s` = "%s"]], k, string.dbencode(v)))
			elseif type(v) == "number" then
				table.insert(tmps, string.format([[`%s` = %s]], k, v))
			elseif type(v) == "boolean" then
				table.insert(tmps, string.format([[`%s` = %d]], k, v and 1 or 0))
			else
				assert(false)
			end
		end
	end
	table.insert(conds, string.format("(%s)", table.concat(tmps, " OR ")))
end
...
return function(tblName, db)
	...
	function Model.get_fields()
		if not Model._fields then
			...
			-- log("Model.get_fields Model._fields =", Model._fields)
		end
	...
	local function make_condition(query, options)
		...
		for k,v in pairs(query) do
			...
			elseif type(v) == "table" then
				...
				if type(v) == "string" then
					table.insert(conds, string.format([[`%s` = "%s"]], k, string.dbencode(v)))
				elseif type(v) == "number" then
					table.insert(conds, string.format("`%s` = %s", k, v))
				elseif type(v) == "boolean" then
					table.insert(conds, string.format("`%s` = %d", k, v and 1 or 0))
				else
					assert(false)
				end
			end
			...
			if option then
				...
				for _,field in ipairs(fields) do
					...
					if char == string.byte("-") then
						...
						table.insert(sort_options, string.format("`%s` DESC", field))
					elseif char == string.byte("+") then
						field = string.sub(field, 2)
						table.insert(sort_options, string.format("`%s` ASC", field))
					else
						table.insert(sort_options, string.format("`%s` ASC", field))
					end
				end
				table.insert(option_conds, string.format("ORDER BY %s", table.concat(sort_options, ",")))
			end
			option = options["limit"]
			if option then
				options["limit"] = nil
				if options["skip"] then
					table.insert(option_conds, string.format([[LIMIT %s, %s]], options["skip"], option))
					options["skip"] = nil
				else
					table.insert(option_conds, string.format([[LIMIT %s]], option))
				end
			end
		...
		if #conds == 0 then
			if not option_conds then
				...
				sql_cond = string.format(" %s", table.concat(option_conds, " "))
			end
		end
		if not option_conds then
			sql_cond = string.format([[WHERE %s]], table.concat(conds, " AND "))
		else
			sql_cond = string.format([[WHERE %s %s]], table.concat(conds, " AND "), table.concat(option_conds, " "))
		end
		return sql_cond
	end
